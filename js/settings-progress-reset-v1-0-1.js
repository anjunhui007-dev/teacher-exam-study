...
function inject(){
 ...
 sec.innerHTML=`<h3>과목별 진행도 초기화</h3><p class="muted">가리기 설정·누적 정답률은 유지하고 회독 진행 정보만 초기화합니다.</p><div style="display:grid;grid-template-columns:minmax(160px,1fr) minmax(220px,1.35fr) auto;gap:8px;align-items:end"><label style="display:grid;gap:6px"><span style="font-size:12px;font-weight:700">과목</span><select id="v101ResetSubject">${subjects.map(s=>`<option value="${esc(s.id)}">${esc(s.name)}</option>`).join('')}</select></label><label style="display:grid;gap:6px"><span style="font-size:12px;font-weight:700">초기화 범위</span><select id="v101ResetMode"><option value="current">현재 회독 진행도만 초기화</option><option value="all">회독 횟수까지 전부 초기화</option></select></label><button type="button" id="v101ResetBtn" class="ghost-btn">초기화</button></div><p id="v101ResetInfo" class="muted" style="margin:8px 0 0;font-size:12px"></p>`;
